...
def build_implementation_prompt(root: Path) -> str:
    ...
    lines.extend(
        [
            ...
            f"- Treat the configured maximum of {max_commits} commit{'s' if max_commits != 1 else ''} as a hard safety ceiling, not a target.",
            "- Prefer small, logically isolated commits when multiple commits improve reviewability; each commit should represent a coherent change rather than a meaningless micro-commit.",
            "- Do not combine unrelated changes merely to reduce the number of commits.",
            f"- If implementation approaches the {max_commits}-commit ceiling, stop and reassess or report that the task may be exceeding its intended scope rather than creating oversized commits to stay under the limit.",
