...
Verification fails closed when configuration is invalid, contains unknown keys in a recognized schema table, is uncommitted, staged, unstaged, changed after session start, or historically touched during the active session; when the session commit budget is exceeded; when scope is violated; when tracked paths use Git index flags that suppress normal change visibility; when any tracked path has an active Git `filter` attribute; when local session state is tracked or changes during verification; when a working directory escapes the repository; when a command times out; when a configured command cannot be launched; or when any configured command fails.
...
When `ekzd start` creates a session, it records the digest of the raw committed `.ekzd/project.toml` blob. While that session is active, `ekzd context`, `ekzd prompt`, `ekzd task`, `ekzd verify`, and `ekzd finish --accept` require the path to remain clean and the committed blob digest to match the recorded digest. If configuration must change, abort or finish the session first, edit and commit the configuration outside an active session, then start a fresh session.
...
At session start EkzD reads `remote.origin.url` when available, sanitizes it, and stores the resulting repository identifier in local session metadata. Userinfo, credentials, query parameters, and fragments are excluded from the rendered identifier. If no origin exists, the session records that no remote locator was available rather than inventing one. Because the value is captured at session start, changing or removing `origin` later does not change either `ekzd prompt` or `ekzd task`.
...
## Deterministic worker-task export

`ekzd task` exports task schema version 2 directly from an active trusted maintainer session. It is complementary to `ekzd prompt`: the prompt remains the semantic implementation handoff, while the task manifest carries only fields the current worker schema mechanically enforces.

Before exporting authority, EkzD requires an active session, enforces the same committed session contract and protected-session-history checks used by the semantic handoff, requires the recorded starting worktree to have been clean and reproducible, validates the frozen V1.1 workflow metadata, and obtains the exact executing runtime build identity. A dirty legacy baseline, invalid or inactive session, protected-history violation, or unavailable runtime identity blocks export and produces no usable manifest.

The exported manifest contains exactly the frozen objective, session-start `HEAD`, declared implementation branch, configured maximum commit count, `scope.include`, `scope.exclude`, acceptance criteria, optional sanitized repository identifier captured at session start, and `[ekzd]` version/build pin. When the session recorded no origin, `repository` is omitted instead of invented. Project verification steps are not copied into task verification because worker checking already loads trusted project verification from the frozen baseline; task verification remains separately additive. Declared sources, prose constraints, and authority prose are also omitted because task schema v2 does not enforce those keys.

Manifest rendering uses one stable field/table order, UTF-8 text, LF newlines, TOML string escaping, and exactly one trailing newline. It excludes mutable current `HEAD`, current worktree contents, timestamps, output paths, absolute installation paths, and other environment-specific values, so the same unchanged session and executing EkzD build produce the same bytes regardless of current directory or external output destination. The result round-trips through the existing task parser without changing schema-v1 or schema-v2 semantics.

With no output option, `ekzd task` emits only the canonical TOML bytes to stdout, making redirection and piping safe from presentation text. `ekzd task --output <path>` requires the destination to resolve outside the project root. Existing identical regular-file content succeeds idempotently without rewriting; an existing different file, symlink, non-regular destination, or in-project path fails. New file publication is staged into a temporary file in the destination directory and published without overwriting an existing destination, so failure does not leave a partial requested task file. Apart from that explicit external file, task export is read-only with respect to project files, Git state, verification state, and `.ekzd/session.json`.
...
V1.1 stores the explicit implementation branch and frozen sanitized repository identity in this local session state alongside the objective, baseline Git state, session policy, verification evidence, and acceptance state. The semantic prompt and machine worker-task export therefore do not need to infer those values from later mutable Git configuration.
...
`ekzd status` is advisory workflow guidance layered on top of the same fail-closed trust checks. For an active V1.1 session it distinguishes the baseline branch and `HEAD`, declared implementation branch, and current branch rather than collapsing them into one ambiguous branch value. A newly started unchanged session points the operator to both `ekzd prompt` and `ekzd task` as complementary handoff artifacts.
...
In the recommended V1.1 workflow, the user and maintainer/planning session define the objective and contract. The operator starts a clean local EkzD session with an explicit task branch. `ekzd prompt` renders the semantic implementation handoff, while `ekzd task` exports the complementary schema-v2 worker authority. A separate implementation session consumes the applicable handoff artifacts, operates from the exact frozen baseline on the declared task branch, implements the scoped change, runs available checks, fixes failures, self-reviews, and pushes/updates the task branch and pull request when its environment supports those repository operations.

The semantic implementation handoff explicitly prohibits merging. If the implementation environment cannot push or create/update a PR, it must report that limitation instead of claiming the operation happened. Authoritative `ekzd verify` remains in the maintainer environment that owns `.ekzd/session.json`, and final merge/reject authority remains with the operator after review.
...
Free-text authority, source, and constraint entries remain instructions for the AI or human operator. They stay in `ekzd prompt` and are intentionally not invented as task-manifest fields. The V1 verifier mechanically enforces canonical configuration integrity, protected harness-history integrity, local session-state integrity, supported Git visibility, historical and current path scope, session commit budget, configured commands, exact-state binding, and explicit acceptance.
...
Future schema versions may add stronger evidence or policy gates, but V1 should remain backward-compatible within `.ekzd/project.toml` `schema_version = 1`. Stateless worker task manifests separately support task schema versions 1 and 2; deterministic export uses existing task schema v2 and does not introduce a new schema version.
